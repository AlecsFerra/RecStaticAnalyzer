...
import Analysis.Environment (Environment, fromList, keys, lookup)
import Analysis.StrictnessResults (StrictnessResults (StrictnessResults), pretty)
import Data.Lattice ((/\), (\/), BoundedMeetSemiLattice (bottom), BoundedJoinSemiLattice (top))
...
makeResults' :: FEnv -> [(FunctionIdentifier, [(VariableIdentifier, Strictness)])]
makeResults' fenv = makeFnResult <$> keys fenv
  where
    makeFnResult id = (id, makeArgs $ lookup id fenv)

makeArgs :: Function -> [(VariableIdentifier, Strictness)]
makeArgs (Function vars sem _) = fmap (makeArg vars sem) vars
  where
